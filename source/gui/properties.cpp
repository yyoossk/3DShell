...
namespace GUI {
    ...
    void DisplayProperties(MenuItem *item) {
        ...
        C2D::Text(((320 - (properties_dialog.subtex->width)) / 2) + 6, ((240 - (properties_dialog.subtex->height)) / 2) + 13, 0.42f, cfg.dark_theme? TITLE_COLOUR_DARK : TITLE_COLOUR, "プロパティ");

        C2D::Textf(66, 57, 0.42f, cfg.dark_theme? TEXT_MIN_COLOUR_DARK : TEXT_MIN_COLOUR_LIGHT,
            cfg.cwd.length() > 22? "親: %.22s..." : "親: %s", cfg.cwd.c_str());
        if (!(item->entries[item->selected].attributes & FS_ATTRIBUTE_DIRECTORY)) {
            ...
            C2D::Textf(66, 73, 0.42f, cfg.dark_theme? TEXT_MIN_COLOUR_DARK : TEXT_MIN_COLOUR_LIGHT, "サイズ: %s", utils_size);
        }
